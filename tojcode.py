...
while k!=len(data):
    ...
    while True:
        ans=0
        #print 'start:'+start
        req_answer=urllib2.Request('http://acm.tju.edu.cn/toj/status.php?pid='+id+'&accept=1&start='+start)
        ...
        while i!=len(data_answer):
            ...
            if data_answer[i]==username:
                ...
                try:
                    ...
                    code = re.findall('(?<=<pre>)([\s\S]*)(?=</pre>)',code_html)[0]
                    titlereq=urllib2.Request('http://acm.tju.edu.cn/toj/showp'+id+'.html')
                    titlehtml=urllib2.urlopen(titlereq).read().decode('utf-8','ignore')
                    title=re.findall('(?<=<title>)([\s\S]*?)(?=\|)',titlehtml)[0]
                    print title
